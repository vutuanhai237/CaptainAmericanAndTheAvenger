...
void BossWizardIdleRoad::Update(float dt)
{
	...
	{
		// tạo ngoại cảnh
		if ((boss->previous_state == 7 || boss->previous_state == 2) && boss->IsOnAir == false) 
		{
	...
	if (abs(boss->GetPosition().y - Player::GetInstance()->GetPosition().y) > 10.0f && IsChamDatLanDau) 
	{
		if (boss->GetPosition().x > Player::GetInstance()->GetPosition().x)
		{
			boss->SetMoveDirection(Entity::Entity_Direction::RightToLeft);
	...
	// Ở gần thì đấm
	if (abs(boss->GetPosition().x - Player::GetInstance()->GetPosition().x) < 20.0f	&& IsChamDatLanDau)
	{
